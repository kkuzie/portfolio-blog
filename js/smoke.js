...
function splat( x, y, dx, dy, color ) {
    ...
    gl.uniform3f( splatProgram.uniforms.color, color[ 0 ] * 0.1, color[ 1 ] * 0.1, color[ 2 ] * 0.3 );
    blit( density.second[ 1 ] );
    density.swap();
...
var count    = 0;
var colorArr = [ Math.random() + 0.1, Math.random() + 0.2, Math.random() + 0.1 ];

canvas.addEventListener( 'mousemove', function ( e ) {

    count++;

    ( count > 25 ) && (colorArr = [ Math.random() + 0.1, Math.random() + 0.2, Math.random() + 0.1 ], count = 0);
